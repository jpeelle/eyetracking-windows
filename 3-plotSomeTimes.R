...
# First filter for start time; then for durations

df <- dfAll[dfAll$startTime==1300,]
df <- df %>% filter(windowLength %in% windowLengths)
#df <- df[df$windowLength>windowLengthMin,]
#df <- df[df$windowLength<windowLengthMax,]
...
#---- plot p values grouped by effect, colored by windowLength ----

df$windowLength <- as.factor(df$windowLength)
ggbarplot(df, x = "term", y = "p.value", order = cond,
